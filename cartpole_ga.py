...
class Ann(Sequential):
    ...
    def __init__(self, child_weights=None):
        ...
        if child_weights is None:
            self.add(Dense(5, input_dim=ninputs, activation='relu'))
            self.add(Dense(5, activation='relu'))
            self.add(Dense(5, activation='sigmoid'))
        else:
            self.add(Dense(5, input_dim=ninputs, activation='relu', weights=[child_weights[0], np.ones(5)]))
            self.add(Dense(5, activation='relu', weights=[child_weights[1], np.ones(5)]))
...
def crossover(ann1, ann2):
    ...
    for i in range(len(ann1_weights)):
        ...
        for j in range(split, np.shape(ann1_weights[i])[1]-2):
            ann1_weights[i][:, j] = ann2_weights[i][:, j] # Maybe it will be neccesary to use copy.deepcopy
